# routerich-bcw-manager
Удобный CLI-менеджер для запуска и автоматизации оригинального blockcheckw на роутерах RouteRich.

Основа работы — оригинальный проект blockcheckw от rcd27.  
Огромная благодарность автору за мощный и полезный инструмент.  
Отдельная благодарность bol-van за оригинальный zapret2.  

Официальный репозиторий blockcheckw: https://github.com/rcd27/blockcheckw  
Если вам нужен сам движок проверки стратегий - используйте оригинальный проект.  
Официальный репозиторий zapret2 - https://github.com/bol-van/zapret2  

Этот репозиторий содержит только оболочку-менеджер для удобного использования на роутерах RouteRich.

Специально адаптировано для роутеров RouteRich.

Проверено на устройстве RouteRich с 512 MB RAM.
Важно: использование 256 потоков может приводить (и приведёт) к нехватке памяти и остановке выполнения скрипта.
Рекомендуемое значение - 128 потоков.

## Версия движка зафиксирована

Менеджер работает с **blockcheckw v0.11.1** — версия захардкожена в скрипте
(переменная `BCW_VERSION`), автополучение `releases/latest` убрано.

Причина: в новых версиях автор изменил способ поиска `nfqws2`, и менеджер перестал
работать. По **v0.11.1 включительно** blockcheckw ищет движок автоподбором по пути
`/opt/zapret2/binaries/<arch>/nfqws2` — именно этот симлинк создаёт менеджер при
установке. Начиная с **v0.11.2** путь жёстко зашит в `/opt/zapret2/nfq2/nfqws2`,
автоподбор убран, и на роутерах, где бинарь лежит не там, движок падает с
`nfqws2 not found` (код возврата 6) ещё до начала сканирования.

Дополнительно в v0.12.0 изменился смысл `check`: `--timeout` стал жёстким потолком
(по умолчанию 3 с вместо 6 с) и добавлен агрессивный `--idle-ms`, из-за чего прогон
ведёт себя иначе и может завершаться без рабочих стратегий там, где старые версии
их находили.

Обновлять движок выше v0.11.1 **не рекомендуется**. Если всё же нужно — проверьте,
что `blockcheckw scan` на роутере проходит этап `Checking prerequisites` без ошибок.

## Известная особенность: встроенный корпус и `--from-file`

На v0.11.1 встроенный корпус стратегий (8648 штук для `tls12`) может отрабатывать
с `errors: 8648` за доли секунды — то есть без реальных проб. Те же самые стратегии,
поданные через `--from-file`, работают нормально (реальные пробы по ~2 с каждая,
`errors: 0`).

Проверено на RouteRich: одни и те же строки корпуса дают
`completed: 8648 | errors: 8648 | 0.4s` при обычном `scan` и
`completed: 10 | failed: 10 | errors: 0 | 2.4s` при `scan --from-file`.
Это особенность самого движка в этой версии, а не проблема менеджера.

Практический вывод: если `scan` отработал подозрительно быстро и вернул 0 стратегий —
это ещё не значит, что обход невозможен. Проверяйте домен вторым способом, например
`scan --from-file` с заранее подготовленным списком стратегий, или запускайте
`check` по имеющемуся vanilla-отчёту.

## Возможности

- установка / обновление blockcheckw
- быстрый поиск стратегий
- полный поиск (scan + check)
- автоматическая остановка / восстановление zapret2
- выбор протокола (http / tls12 / tls13)
- выбор DNS режима
- сохранение рабочих стратегий в файл
- просмотр результатов
- очистка логов и отчётов
- запуск одной командой: `bcw`

## Установка

```bash
wget -O /tmp/install.sh https://raw.githubusercontent.com/dedikar/routerich-bcw-manager/main/install.sh && sh /tmp/install.sh
```

## Удаление

```bash
wget -O /tmp/uninstall.sh https://raw.githubusercontent.com/dedikar/routerich-bcw-manager/main/uninstall.sh && sh /tmp/uninstall.sh
```

## Использование

После установки откроется окно менеджера  
<img width="696" height="299" alt="image" src="https://github.com/user-attachments/assets/588e5e2c-e41c-4b06-9cd0-46d435711e73" />

Перейдите в настройки и введите желаемый домен для поиска стратегии, выберите желаемый(е) протокол(ы)  
<img width="353" height="268" alt="image" src="https://github.com/user-attachments/assets/7d30b6f1-6945-45f2-9a43-8e17a9b98a9d" />

Запустите быстрый, либо полный поиск. 
Найденные стратегии будут сохранены в /root/, также возможен просмотр стратегий из меню скрипта.
